...
import { FIELDS, CONTRIBUTOR_FIELDS } from "@/constants";
...
const PRESENT_FIELDS = CONTRIBUTOR_FIELDS.present;
const DISTRIBUTION_FIELDS = CONTRIBUTOR_FIELDS.distribution;

export async function fetchContributors(clientId: string) {
  ...
  const presentSlice = (start?: number, end?: number) =>
    PRESENT_FIELDS.slice(start, end)
      .map(findInPresent)
      .filter((item) => !!item)
      .map(toPresentProperty);
  ...
  const formatted = {
    ...
    contributorType: distribution(0),
    nameIdentifier: toPresentProperty(findInPresent(PRESENT_FIELDS[5])!),
    nameIdentifierScheme: distribution(1),
    affiliation: presentSlice(-2),
    affiliationIdentifierScheme: distribution(2),
  };

  return formatted;
}

export default function useContributors(clientId: string) {
  const query = useQuery({
    queryKey: [clientId, "contributors"],
    queryFn: () => fetchContributors(clientId),
  });
